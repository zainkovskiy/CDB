...
class Search{
    ...
    for (let box of this.containerAll){
      box.addEventListener('click', event => {
        if (event.target.tagName === 'P'){
          ...
          if (event.target.dataset.input === 'reqHouseDeveloper'){
  ...
  checkRoomPart(){
    const checkBoxRoom = document.querySelector(`INPUT[name='room']`);
    if (checkBoxRoom){
...
function dadata(){
  const token = "408e6651c0b9bfc8e2f487383d45353973f3285c";
  const type = "ADDRESS";
